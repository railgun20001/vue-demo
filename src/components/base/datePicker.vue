<template>
  <div class="datetime-picker" :style="{ width: width }">
    ...
    v-model="showValue"
    @click="show = !show"/>
    <div class="picker-wrap" v-if="show">
...
export default {
  ...
  methods: {
    ...
    update () {
      ...
      var curFirstDay = time.getDay()         // 当月第一天星期几
      curFirstDay === 0 && (curFirstDay = 7)
      time.setDate(0)                         // 上个月的最后一天
      ...
      time.setMonth(time.getMonth() + 2, 0)    // 当月的最后一天
      var curDayCount = time.getDate()
      time.setDate(1)
      var value = this.value || this.stringify(new Date())    // 当前时间的YYYY-MM-DD（可自定format属性）
      for (let i = 0; i < curDayCount; i++) {
        let tmpTime = new Date(time.getFullYear(), time.getMonth(), i + 1)   // 判断是不是当天
        let status = ''
